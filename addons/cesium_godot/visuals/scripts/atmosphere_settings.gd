...
@export var in_scattering_points : int = 30 :
	set(val):
...
var optical_depth_dirty : bool = false
@export var optical_depth_texture: ImageTexture


func on_data_changed():
	#optical_depth_dirty = true
	emit_changed()
...
func set_properties(material : Material, body_radius : float):
	...
	if optical_depth_dirty or optical_depth_texture == null:
		optical_depth_compute()
		optical_depth_dirty = false
...
func optical_depth_compute():
	if not rd:
		...
		# Load GLSL shader
		var shader_optical_depth: RDShaderFile = load("res://addons/globe-rendering/visual_effects/atmosphere/AtmosphereTexture.glsl")
		var shader_spirv: RDShaderSPIRV = shader_optical_depth.get_spirv()
